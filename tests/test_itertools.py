...
def test_count__advanced():
    ...
    power_exponents = curried.pipe(
        itertools.count(-5), takewhile(lambda x: x <= 4), tuple
    )
    ...
    actual = curried.pipe(
        itertools.count(0), curried.filter(bumbag.iseven), curried.take(5), tuple
    )
    expected = (0, 2, 4, 6, 8)
    assert actual == expected

    actual = curried.pipe(
        itertools.count(0), curried.filter(bumbag.isodd), curried.take(5), tuple
    )
    expected = (1, 3, 5, 7, 9)
    assert actual == expected
