...
def test_accumulate():
    ...
    actual = functoolz.pipe(
        [1, 2, 3], curried.accumulate(operator.add, initial=-1), tuple
    )
    expected = (-1, 0, 2, 5)
    assert actual == expected
...
def test_get(n10):
    ...
    actual = curried.get("Alice", phonebook)
    expected = "555-1234"
    assert actual == expected

    actual = curried.get("David", phonebook, None)
    expected = None
    assert actual == expected

    actual = curried.get(["Alice", "Bob"], phonebook)
...
def test_sliding_window():
    ...
    actual = functoolz.pipe(
        [1, 2, 3, 4], curried.sliding_window(2), curried.map(statistics.mean), tuple
    )
...
class TestRecipes:
    def test_countby(self):
        ...
        actual = functoolz.pipe(
            integer_sequence(1), curried.take(11), curried.countby(is_even)
        )
...
class TestDictToolz:
    ...
    def test_itemmap(self):
        ...
        actual = curried.itemmap(reversed, accountids)
